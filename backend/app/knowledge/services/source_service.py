...
class SourceService:
    ...
    async def upload_source(
        ...
        try:
            ...
            if isinstance(e, (HTTPException, SourceServiceError)):
                ...
                raise SourceServiceError(f"Upload failed: {str(e)}")
    ...
    async def _process_source_async(
        ...
        """
        try:
            await self._process_source(file_path, source_doc)
        except Exception as e:
            logger.error(f"Background processing failed for {source_doc.id}: {str(e)}")
            source_doc.mark_as_failed(str(e))
            # TODO: Update database record

    async def _process_source(self, file_path: str, source_doc: SourceDocument) -> None:
        """
        ...
        try:
            logger.info(
                f"Starting processing for {source_doc.filename} ({source_doc.id})"
            )
            ...
            # Create text chunks
            chunks = await self._create_text_chunks(content, source_doc.id)
            ...
            logger.error(f"Processing failed for {source_doc.filename}: {str(e)}")
            source_doc.mark_as_failed(str(e))
            # TODO: Update database record
            raise FileProcessingError(f"Processing failed: {str(e)}")

    async def _extract_text_content(
        self, file_path: str, doc_type: Optional[DocumentType]
    ) -> str:
        """
        ...
        try:
            ...
            elif file_ext == "txt" or doc_type == DocumentType.TXT:
                return await self._extract_text_content_plain(file_path)
            elif file_ext in ["md", "markdown"] or doc_type == DocumentType.MARKDOWN:
                return await self._extract_text_content_plain(file_path)
            elif file_ext == "docx" or doc_type == DocumentType.DOCX:
                return await self._extract_docx_content(file_path)
            elif file_ext in ["html", "htm"] or doc_type == DocumentType.HTML:
                return await self._extract_html_content(file_path)
            elif file_ext == "json" or doc_type == DocumentType.JSON:
                return await self._extract_json_content(file_path)
            elif file_ext == "csv" or doc_type == DocumentType.CSV:
    ...
    async def _extract_html_content(self, file_path: str) -> str:
        ...
        try:
            ...
            raise FileProcessingError("BeautifulSoup4 is required for HTML processing")
        except Exception as e:
            raise FileProcessingError(f"HTML processing failed: {str(e)}")
    ...
    async def _create_text_chunks(
        ...
        try:
            ...
            raise FileProcessingError(f"Text chunking failed: {str(e)}")
    ...
    async def _process_embeddings(
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Generated {len(embeddings)} embeddings using {embedding_service.model_name}"
                )
            except EmbeddingServiceError as e:
                logger.warning(
                    f"Custom embedding generation failed: {str(e)}, using ChromaDB default embeddings"
                )
                embeddings = None  # Let ChromaDB handle embeddings
            ...
            vector_ids = await vector_store.add_documents(
                ...
                embeddings=embeddings,  # Pass pre-computed embeddings or None
                metadatas=metadatas,
                ids=chunk_ids,
            )
            ...
            raise FileProcessingError(f"Vector store operation failed: {str(e)}")
        except Exception as e:
            raise FileProcessingError(f"Embedding processing failed: {str(e)}")
    ...
    async def delete_source_document(self, source_id: str) -> bool:
        """
        Delete source document and associated data.

        Args:
            source_id: Document ID to delete
        ...
        try:
            ...
            source_doc = await self.get_source_document(source_id)
            if not source_doc:
                return False
            ...
            if source_doc.file_path and os.path.exists(source_doc.file_path):
                os.unlink(source_doc.file_path)
            ...
            logger.error(f"Failed to delete source document {source_id}: {str(e)}")
            raise SourceServiceError(f"Delete failed: {str(e)}")
