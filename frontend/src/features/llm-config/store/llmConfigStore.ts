        ...
        persist(
            (set, get) => ({
                ...
                fetchConfigurations: async () => {
                    ...
                    try {
                        ...
                        if (!currentConfig) {
                            ...
                            set({ configurations, currentConfig: defaultConfig, loading: false });
                        } else {
                ...
                createConfiguration: async (config) => {
                    set({ loading: true, error: null });
                    try {
                        const newConfig = await llmConfigService.createConfiguration(config);
                        const configurations = [...get().configurations, newConfig];

                        // If this is the first config or marked as default, set it as current
                        const shouldSetAsCurrent = configurations.length === 1 || newConfig.isDefault;

                        set({
                            configurations,
                            currentConfig: shouldSetAsCurrent ? newConfig : get().currentConfig,
                            loading: false
                        });
                ...
                updateConfiguration: async (id, updates) => {
                    set({ loading: true, error: null });
                    try {
                        const updatedConfig = await llmConfigService.updateConfiguration(id, updates);
                        ...
                        const currentConfig = get().currentConfig;
                        const newCurrentConfig = currentConfig?.id === id ? updatedConfig : currentConfig;
                ...
                deleteConfiguration: async (id) => {
                    ...
                    try {
                        ...
                        const currentConfig = get().currentConfig;
                        let newCurrentConfig = currentConfig?.id === id ? null : currentConfig;

                        if (!newCurrentConfig && configurations.length > 0) {
                            newCurrentConfig = configurations.find(c => c.isDefault) || configurations[0];
                        }
                ...
                setDefaultConfiguration: async (id) => {
                    set({ loading: true, error: null });
                    try {
                        const updatedConfig = await llmConfigService.setDefaultConfiguration(id);
                        ...
                        set({
                            error: error instanceof Error ? error.message : 'Failed to set default configuration',
                            loading: false
                        });
                        throw error;
                    }
                ...
                testConfiguration: async (id, testRequest) => {
                    try {
                        const result = await llmConfigService.testConfiguration(id, testRequest);
