...
const SourceList: React.FC<SourceListProps> = ({
    ...
    const filteredSources = useMemo(() => {
        return sources.filter((source: KnowledgeSource) => {
            const matchesSearch = source.filename.toLowerCase().includes(searchText.toLowerCase());
            const matchesStatus = statusFilter === 'all' || source.status.status === statusFilter;
            const fileExtension = source.filename.split('.').pop()?.toLowerCase() || '';
    ...
    const fileTypes = useMemo(() => {
        const types = new Set(
            sources.map((source: KnowledgeSource) => source.filename.split('.').pop()?.toLowerCase() || 'unknown')
        );
