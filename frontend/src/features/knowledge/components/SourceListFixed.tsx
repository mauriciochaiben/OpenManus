...
const SourceList: React.FC<SourceListProps> = ({
    ...
    const getDropdownItems = (source: KnowledgeSource): MenuProps['items'] => [
        ...
        ...(source.status && (typeof source.status === 'object' ? source.status.status : source.status) === 'failed' ? [{
            key: 'retry',
                ...
                <List.Item.Meta
                    ...
                    description={
                        <Space direction="vertical" style={{ width: '100%' }} size="small">
                            {/* Processing Progress */}
                            {source.status && typeof source.status === 'object' && source.status.status === 'processing' && source.status.progress && (
                                <Progress
                            ...
                            <Space wrap>
                                ...
                                {source.status && typeof source.status === 'object' && source.status.last_updated && (
                                    <Space size="small">
                            ...
                            {/* Error Message */}
                            {source.status && typeof source.status === 'object' && source.status.status === 'failed' && source.status.error_message && (
