...
const SourceSelector: React.FC<SourceSelectorProps> = ({
    ...
    const getFileIcon = (source: KnowledgeSource) => {
        if (source.file_type === 'application/pdf' || source.filename.toLowerCase().endsWith('.pdf')) {
    ...
    const formatSourceLabel = (source: KnowledgeSource) => {
        ...
        const displayName = source.filename.length > maxLength
                        ...
                        {selectedSources.map(source => (
                            ...
                            >
                                <Tag
                                    closable
                                    onClose={() => {
                                        const newSelection = selectedSourceIds.filter(id => id !== source.id);
        ...
        <Space direction="vertical" style={{ width: '100%' }} size="small">
                ...
                <>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Space size="small">
                            ...
                            <Text strong style={{ fontSize: '14px' }}>Knowledge Context</Text>
                            <Tooltip title="Select documents to provide additional context for AI responses">
                                <InfoCircleOutlined style={{ color: '#666', fontSize: '12px' }} />
                            </Tooltip>
                        </Space>
                        <Space size="small">
            ...
            <Select
                ...
                filterOption={(input, option) => {
                    ...
                    return source?.filename.toLowerCase().includes(input.toLowerCase()) || false;
                }}
                ...
                notFoundContent={
                    loading ? (
                        <div style={{ textAlign: 'center', padding: '12px' }}>
                            <Spin size="small" />
                            <div style={{ marginTop: '4px', fontSize: '12px' }}>Loading sources...</div>
                        </div>
            ...
            {!compact && selectedSourceIds.length === 0 && completedSources.length > 0 && (
                <Text type="secondary" style={{ fontSize: '11px' }}>
                    💡 Select sources to provide context and improve AI response accuracy
                </Text>
            )}
        ...
        return (
            <Card
                ...
                style={{
                    ...
                    border: selectedSourceIds.length > 0 ? '1px solid #1890ff' : undefined
                }}
