    ...
    private handleIncomingMessage(message: WebSocketMessage): void {
        ...
        switch (message.type) {
            ...
            case 'task_failed':
                ...
                break;
            case 'task_progress':
                this.eventBus.emitTaskProgress(message.data.taskId, message.data.progress);
                break;
            case 'notification':
...
const clientId = `frontend-alt-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
export const webSocketManager = new WebSocketManager(`ws://localhost:8000/api/v2/chat/ws/${clientId}`);
