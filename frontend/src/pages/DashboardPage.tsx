...
const DashboardPage: React.FC = () => {
    ...
    const loadDashboardData = async () => {
        try {
            ...
            const [dashboardStats, health] = await Promise.all([
                ...
                systemApi.getHealth().catch(() => ({ status: 'unknown', version: 'unknown' }))
            ]);
            ...
            const recentTasksFromActivity = dashboardStats.recent_activity.map((activity: any) => ({
                id: activity.task_id,
    ...
    return (
        <div style={{ padding: '24px' }}>
            ...
            {systemHealth && (
                <Alert
                    message={`Sistema ${systemHealth.status === 'healthy' ? 'Online' : 'Offline'}`}
                    ...
                    >
                        ...
                        {stats.errorTasks > 0 && (
                            <div style={{ marginTop: '16px', textAlign: 'center' }}>
                                <Tag color="error">
                                    {stats.errorTasks} tarefa(s) com erro
                                </Tag>
                            </div>
                        )}
                    </Card>
