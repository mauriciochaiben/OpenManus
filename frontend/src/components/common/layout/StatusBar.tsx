...
const StatusBar: React.FC = () => {
    ...
    const getStatusColor = (statusType: 'connected' | 'disconnected' | 'connecting') => {
        switch (statusType) {
        ...
        <div className="status-bar">
            <Space size="large">
                <div className="status-item">
                    <Badge
                        status={status.websocket === 'connected' ? 'success' :
                            status.websocket === 'connecting' ? 'processing' : 'error'}
                    />
                    <WifiOutlined style={{ color: getStatusColor(status.websocket) }} />
                    <Text style={{ color: getStatusColor(status.websocket) }}>
                        WebSocket: {status.websocket === 'connected' ? 'Conectado' :
                            status.websocket === 'connecting' ? 'Conectando' : 'Desconectado'}
                    </Text>
                </div>

                <div className="status-item">
                    <Badge status={status.backend === 'connected' ? 'success' : 'error'} />
                    <CloudServerOutlined style={{ color: getStatusColor(status.backend) }} />
                    <Text style={{ color: getStatusColor(status.backend) }}>
                        Backend: {status.backend === 'connected' ? 'Conectado' : 'Desconectado'}
                    </Text>
                </div>
